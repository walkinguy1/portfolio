...
class InfiniteGridMenu {
  ...
  #deltaFrames = 0;
  #frames = 0;
  // ADDED: lets React stop the render loop on unmount (see dispose()).
  #disposed = false;
  #rafId = null;
  ...
  run(time = 0) {
    // ADDED: bail once React has unmounted us, otherwise this rAF chain
    // runs forever against a dead canvas.
  ...
  // ADDED: teardown so the render loop doesn't outlive the component.
  // Deliberately does NOT call WEBGL_lose_context: under StrictMode React
  // mounts, unmounts, then remounts against the same canvas element, and a
  // lost context is handed straight back to the second mount — every GL call
  // then fails. Dropping the canvas is what frees the context.
  dispose() {
    this.#disposed = true;
    if (this.#rafId !== null) {
  ...
  #init(onInit) {
    // CHANGED from the original `alpha: false`: an opaque drawing buffer turns
    // the shader's transparent clear colour into a solid black rectangle, which
    // only blends in on a black page. Straight (non-premultiplied) alpha matches
    // how the fragment shader writes `outColor.a`.
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      // ADDED: the atlas upload is async — skip it if we were unmounted meanwhile.
      if (this.#disposed) return;
  ...
  useEffect(() => {
    ...
    if (canvas) {
      try {
        ...
        // ADDED: the app has no error boundary, so letting this throw would
        // blank the whole page. Hand it to the parent to take us down instead.
        console.error('InfiniteMenu: WebGL setup failed.', error);
        onError?.(error);
        return undefined;
      }
    }
    ...
    return () => {
      ...
      // ADDED: stop the render loop on unmount.
      sketch?.dispose();
    };
  }, [items, scale, onError]);
